...
public class AreaController : MonoBehaviour
{
    ...
    public void EnqueueNPC(NPCController npc)
    {
        if (npc == null) return;
        CleanupQueue();
        // Enforce queue limit if configured (>0)
        if (queueLimit > 0 && npcQueue.Count >= queueLimit)
        {
            // Already full; do not enqueue
            return;
        }
    ...
    public Vector3 GetExitPosition()
    {
        ...
        // Fallback: if there is a queue anchor, move a little forward from it; otherwise move forward from this transform.
        Vector3 basisPos = queueAnchor != null ? queueAnchor.position : transform.position;
        Vector3 forward = queueAnchor != null ? queueAnchor.forward : transform.forward;
        return basisPos + forward * 2f; // default exit: 2 meters forward
    }
